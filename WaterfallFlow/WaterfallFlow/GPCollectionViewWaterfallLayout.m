...
NSString *const GPCollectionElementKindSectionHeader = @"GPCollectionElementKindSectionHeader";
NSString *const GPCollectionElementKindSectionFooter = @"GPCollectionElementKindSectionFooter";

@interface GPCollectionViewWaterfallLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <GPCollectionViewDelegateWaterfallLayout> delegate;
...
@implementation GPCollectionViewWaterfallLayout

/// How many items to be union into a single rectangle
static const NSInteger unionSize = 20;

static CGFloat GPFloorCGFloat(CGFloat value) {
...
- (void)setItemRenderDirection:(GPCollectionViewWaterfallLayoutItemRenderDirection)itemRenderDirection {
...
- (CGFloat)itemWidthInSectionAtIndex:(NSInteger)section {
  ...
  return GPFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
}
...
- (id <GPCollectionViewDelegateWaterfallLayout> )delegate {
  return (id <GPCollectionViewDelegateWaterfallLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
  ...
  _itemRenderDirection = GPCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
  ...
  NSAssert([self.delegate conformsToProtocol:@protocol(GPCollectionViewDelegateWaterfallLayout)], @"UICollectionView's delegate should conform to GPCollectionViewDelegateWaterfallLayout protocol");
  ...
  for (NSInteger section = 0; section < numberOfSections; ++section) {
    ...
    CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
    NSInteger columnCount = [self columnCountForSection:section];
    CGFloat itemWidth = GPFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
    ...
    if (headerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:GPCollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
    ...
    for (idx = 0; idx < itemCount; idx++) {
      ...
      if (itemSize.height > 0 && itemSize.width > 0) {
        itemHeight = GPFloorCGFloat(itemSize.height * itemWidth / itemSize.width);
      }
    ...
    if (footerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:GPCollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
  ...
  switch (self.itemRenderDirection) {
    case GPCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst:
      index = [self shortestColumnIndexInSection:section];
      break;

    case GPCollectionViewWaterfallLayoutItemRenderDirectionLeftToRight:
      index = (item % columnCount);
      break;

    case GPCollectionViewWaterfallLayoutItemRenderDirectionRightToLeft:
